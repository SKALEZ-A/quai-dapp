...
class TransactionManager {
    ...
    async executeTransaction(contract, method, args, options = {}) {
        ...
        const parsed = errorHandler_1.errorHandler.parseError(lastError, {
            operation: `${contract.target}.${method}`,
     ...
     */
    async estimateGasWithFallback(contract, method, args, options = {}) {
        const opts = {
     ...
     */
    async waitForTransaction(tx, timeout) {
        return Promise.race([
            tx.wait(),
     ...
     */
    async validateTransaction(contract, method, args, signer) {
        const issues = [];
        try {
