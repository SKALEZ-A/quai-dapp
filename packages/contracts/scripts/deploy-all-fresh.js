/**
 * COMPREHENSIVE QNS DEPLOYMENT SCRIPT
 * Deploy all QNS contracts to Quai Testnet from scratch
 * 
 * This script:
 * 1. Deploys all core contracts in correct order
 * 2. Grants all necessary roles
...
async function main() {
  try {
    console.log('\n═══════════════════════════════════════════════════════')
    console.log('🚀 QNS FRESH DEPLOYMENT - QUAI TESTNET')
    console.log('═══════════════════════════════════════════════════════\n')
    ...
    // ========================================
    // STEP 1: Deploy QNSRegistry
    // ========================================
    console.log('═══════════════════════════════════════════════════════')
    console.log('📦 [1/4] Deploying QNSRegistry...')
    console.log('═══════════════════════════════════════════════════════')
    
    const registryIpfs = await deployMetadata.pushMetadataToIPFS('QNSRegistry')
    ...
    console.log(`✅ QNSRegistry deployed: ${deployedContracts.QNS_REGISTRY}`)
    
    // Initialize Registry
    console.log('🔧 Initializing QNSRegistry...')
    const initRegistryTx = await registry.initialize(admin)
    await initRegistryTx.wait()
    console.log('✅ QNSRegistry initialized!\n')

    // ========================================
    // STEP 2: Deploy QNSNFT
    // ========================================
    console.log('═══════════════════════════════════════════════════════')
    console.log('📦 [2/4] Deploying QNSNFT...')
    console.log('═══════════════════════════════════════════════════════')
    
    const nftIpfs = await deployMetadata.pushMetadataToIPFS('QNSNFT')
    ...
    console.log(`✅ QNSNFT deployed: ${deployedContracts.QNS_NFT}`)
    
    // Initialize NFT
    console.log('🔧 Initializing QNSNFT...')
    const initNFTTx = await nft.initialize(admin, 'Quai Name Service', 'QNS')
    await initNFTTx.wait()
    console.log('✅ QNSNFT initialized!\n')

    // ========================================
    // STEP 3: Deploy QNSReservedNames
    // ========================================
    console.log('═══════════════════════════════════════════════════════')
    console.log('📦 [3/4] Deploying QNSReservedNames...')
    console.log('═══════════════════════════════════════════════════════')
    
    const reservedIpfs = await deployMetadata.pushMetadataToIPFS('QNSReservedNames')
    ...
    const reserved = await ReservedFactory.deploy()
    ...
    console.log(`✅ QNSReservedNames deployed: ${deployedContracts.QNS_RESERVED_NAMES}`)
    
    // Initialize Reserved Names
    console.log('🔧 Initializing QNSReservedNames...')
    const initReservedTx = await reserved.initialize(admin)
    await initReservedTx.wait()
    console.log('✅ QNSReservedNames initialized!\n')
    ...
    console.log('   - Visit: http://localhost:3000/qns/namesearch')
