...
function loadConfig() {
  try {
    ...
    return {
      ...
      registrarAddress: deployedAddresses.contracts.QNS_REGISTRAR,
      nftAddress: deployedAddresses.contracts.QNS_NFT,
      registryAddress: deployedAddresses.contracts.QNS_REGISTRY,
      reservedNamesAddress: deployedAddresses.contracts.QNS_RESERVED_NAMES,
...
async function testContractPermissions(provider, config) {
  ...
  const registrarAbi = JSON.parse(
    fs.readFileSync(path.join(__dirname, 'metadata/QNSRegistrarSimple_metadata.json'), 'utf8')
  ).output.abi;
  
  const nftAbi = JSON.parse(
    fs.readFileSync(path.join(__dirname, 'metadata/QNSNFTSimple_metadata.json'), 'utf8')
  ).output.abi;
  
  const registryAbi = JSON.parse(
    fs.readFileSync(path.join(__dirname, 'metadata/QNSRegistrySimple_metadata.json'), 'utf8')
  ).output.abi;
...
async function runAllTests() {
  ...
  if (!results.contractPermissions) {
    ...
    log('   Run: node scripts/check-and-fix-permissions.js', 'yellow');
  }
